...
idxfiles <- Sys.glob('../*/pipeline/filter_reads/*/*.idxtmp')%>%str_subset('Splicing')
...
samples_idx <- idxfiles%>%map_df(.id='sample',function(idxfile){
	...
	# stopifnot(sample_idx%$%	{sum(reads[tRNA|rRNA|miRNA|snoRNA|spikein])/sum(reads)}%>%`>`(.2))	
	#
	# if(sample_idx[,c('tRNA','rRNA','miRNA','snoRNA','spikein')]%>%rowSums%>%`>`(0)%>%mean%>%`>`(.2)%>%not){
		# stop('more than 80% of contaminants fall in the unkown category')
	# }
...
stop("this is broken here - i cant pull out figures for total reads because some of the libs are hisat. Needs a little bit of logic there")
