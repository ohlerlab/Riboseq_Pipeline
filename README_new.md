...
This is the lab's standard Ribo-seq processing pipeline. It consists of a dockerised snakemake workflow that ties together R scripts, as well as [ORFquant](3), [Ribostan](4) and [RiboseQC](5).  
...
1. **Install Singularity:**  
    ...
    [Link to guide.](1)   
    >*If you have conda installed, it might interfer with Singularity.  
    >Add this to the end of your path ('`.bashrc`' on linux) to avoid this behavior:*
    >```
...
1. Edit '`src/sample_config.tsv`' to point it to your fastq files (zipped or not, either is fine) with the appropriate parameters.

2. Edit '`config.yaml`'.  
This is where you put in for example, the path to annotation, genome sequence, etc. See comments in file.  
...
3. Dry run of the pipeline:  
    ...
    Most likely there'll be misspecified paths or misplaced symbols like **'. ,'**.   
